...
|---|---|---|
| **Bare-metal track: L0 loop LIVE-PROVEN (nested), hardware leg pending** (Phase 3) | The full install loop is validated end-to-end on the lab NUC (2026-08-09): identity POST → allowlisted answer (`validate-answer`-clean) → unattended PVE 9.2 install → post-install webhook (`bm_installed`) → firstboot `pveum` bootstrap → token phoned home → SecretsGroup created → the auto-created token authenticates back into the node with full NFVAutomation privileges. Only the physical vmedia leg (XCC1 PATCH-EXT mount + boot dress rehearsal) awaits a real SE350 | Nested loop DONE; then the discovery job's opt-in checks on a real unit |
