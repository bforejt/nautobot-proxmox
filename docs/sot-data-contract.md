...
| BMC/XCC address | **Settled (2026-08-08)**: a dedicated interface named `xcc` on the physical device (SE350, SE455 V3) with its IP assigned — native, visible, cable-truthful. Never a host NIC: it is excluded from NIC-name pinning | Layout process creates it |
| Install identity (bare-metal loop) | `device.serial` = the DMI system serial the installer POSTs (the answer service's allowlist key); `device_type.model` selects the install profile `bmc/profiles/<slug>.yaml`; CF `provisioning_state=awaiting_install` gates the answer; `primary_ip4` (static) needs a DefaultGW-role IP in its prefix and the carrying interface's `mac_address` for an exact NIC filter | **Settled (2026-08-09)** — [baremetal-install.md](baremetal-install.md) |
| Host NIC names (physical nodes) | Profiles with `install.interface_name_pinning` (SE455 V3) pin every physical NIC's Linux name by MAC at install: a Device interface that records its `mac_address` gets its **Nautobot name** as the Linux name (`mgmt` → `mgmt`), unmapped ports get the installer's `nic<N>`. Names: 2–15 chars, letter first, alnum/underscore, not `nic<N>`, unique | **Settled (2026-09-16, decision #51)** |
