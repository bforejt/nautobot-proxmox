...
| Sanity checks: target server name must match; datastores checked for space and correct targets | Deploy-job pre-flight: Nautobot Device ↔ Proxmox node hostname **and** DMI serial match (refuse on mismatch); storage ID exists, is the intended target, and has capacity — all via API before any write |
