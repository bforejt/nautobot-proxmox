...
|---|---|---|
| **Bare-metal track: L0 loop LIVE-PROVEN on real hardware (PXE) + nested** (Phase 3) | Validated end-to-end twice on 2026-08-09: (a) nested VM on the lab NUC, (b) **a physical Intel NUC over PXE** — netboot → serial-discovery refusal → Device created → unattended PVE 9.2 install to NVMe → firstboot `pveum` bootstrap → token phoned home → SecretsGroup created → the auto-created token authenticates into the node. Both delivery decisions (#41/#42) exercised for real. Remaining: the SE350 vmedia leg (XCC1 PATCH-EXT mount + boot dress rehearsal) | PXE + nested DONE; then the discovery job's opt-in checks on a real SE350 |
