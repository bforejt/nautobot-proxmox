...
> **Shortcut for §§1–3:** the `SE350 Platform Discovery (read-only)` Nautobot Job
...
> attaches the JSON dumps to the JobResult. The curl commands below remain as the
> no-Nautobot fallback.
