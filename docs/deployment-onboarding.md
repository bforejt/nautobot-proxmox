...
|---|---|---|
| **Bare-metal track: EVERY MECHANISM FIELD-PROVEN** (Phase 3) | Validated end-to-end three ways: (a) nested VM on the lab NUC (2026-08-09), (b) **a physical Intel NUC over PXE** (2026-08-09) — netboot → serial-discovery refusal → Device created → unattended PVE 9.2 install → firstboot `pveum` bootstrap → token phoned home, and (c) **the SE350 vmedia leg (2026-08-10, nfvlabspt1)** — XCC1 PATCH-EXT write test (`Inserted=true`, verified eject) + dress-rehearsal boot (one-shot CD override + ForceRestart, ISO boot confirmed on console). Host verification (disk filter, DMI serial, X722, Secure Boot) green on the same unit. Remaining: the first *real* SE350 install — a scheduling decision, not an engineering one. **SE455 V3 (XCC2) support built 2026-09-16**: DeviceType + profile, out-of-band RAID layout over XCC2 Redfish (decision #50), interface name pinning with SoT names (#51), XCC2 Platinum licensing (#49); profile shape confirmed against a hand-built unit's udev dump | Schedule it: Device w/ serial `J101YCEB` + `awaiting_install` + Active prepared-ISO version, vmedia-mount the prepared ISO, watch the state machine. SE455 V3: `Apply Storage Layout` dry run, then the install job — the Redfish volume create, the LVM-thin firstboot step and the pinned names are `[lab-verify]` |
| **RAID layout** *(closed for profiles with a `storage` section)* | The SE455 V3's two mirror sets were an admin's UEFI step; the install job (or `Apply Storage Layout`) now creates them through the XCC2 from the profile — boot = the smaller pair, data = the larger — keeping or adopting hand-made volumes, never deleting. SE350 keeps its factory RAID volume (no `storage` section) | [baremetal-install.md](baremetal-install.md) disk layout; decision #50 |
| **Host baseline + network jobs unbuilt** (L1/L2: bond/bridge/tuning) | Node networking (LACP, VLAN-aware bridge, MTU) is manual today. Interface names, however, are now pinned at install from the SoT (#51: a Device interface with a MAC becomes the Linux name, the rest `nic<N>`), so the future bond/bridge job can reference Nautobot interface names | firstboot hook + `DeployHostNetworkJob` |
